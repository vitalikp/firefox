...
{
  PROFILER_LABEL("nsPluginHost::GetPluginForContentProcess", OTHER);
...
nsPluginHost::SendPluginsToContent()
{
  ...
  for (auto p : parents)
  {
    Unused << p->SendSetPluginList(newPluginEpoch, pluginTags);
  }
  return NS_OK;
}
...
{
  PROFILER_LABEL("nsPluginHost::StopPluginInstance", OTHER);
