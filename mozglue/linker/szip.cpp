                      ...
                      const char *outName, Buffer &outBuf)
{
  ...
  if (dictSize) {
    ...
    for (size_t d = firstDictSize; d <= lastDictSize; d += 4096) {
      DEBUG_LOG("Compressing with dictionary of size %zu", d);
  ...
  DEBUG_LOG("Used filter \"%s\" and dictionary size of %zu",
            filterName[filter], dictSize);
  LOG("Compressed size is %zu", outBuf.GetLength());
  ...
  if (size != origSize) {
    ERROR("Compression error: %zu != %zu", size, origSize);
    return 1;
  }
